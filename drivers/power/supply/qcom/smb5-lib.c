			...
			int icl_ua, const char *client)
{
	...
	smblib_dbg(chg, PR_MISC, "set icl_ua %d uA and get Vpsns = %d uV\n", icl_ua, Vpsns);
				...
				const union power_supply_propval *val)
{
	...
	smblib_dbg(chg, PR_MISC, "thermal level:%d, batt temp:%d, thermal_levels:%d dc_present=%d\n",
				...
				const union power_supply_propval *val)
{
	...
	if (chg->flag_dc_present && val->intval) {
		...
		if (rc < 0)
			dev_err(chg->dev,
					"Couldn't enable secondary chargers  rc=%d\n", rc);
		smblib_dbg(chg, PR_MISC, "enable cp by wireless\n");
	} else
